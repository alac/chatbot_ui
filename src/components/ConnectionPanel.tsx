...
const EditConnectionsPanel = () => {
  ...
  var initialFormatSettings = storageManager
    .getAllFormatSettings()
    .get(currentFormatId);
  ...
  const handleReset = () => {
    const originalFormatSettings =
      getDefaultFormatSettingsMap().get(currentFormatId);
  ...
  const handleDelete = () => {
    ...
    const defaultFormatSettings = storageManager
      .getAllFormatSettings()
      .get(defaultFormatId);
      ...
      <DialogOverlay>
        ...
        >
            ...
            >
              ...
              <SelectPopover>
                <SelectContent aria-label="items" className="px-2 py-2">
                  <SelectItem textValue="OpenAI" id="OPENAI">
                    OpenAI /Completions
                  </SelectItem>
                  <SelectItem textValue="Dummy" id="DUMMY">
                    Dummy
                  </SelectItem>
                </SelectContent>
              </SelectPopover>
          ...
          <div
            ...
            key={"FormatSelector_" + currentFormatId}
          >
            <TextField className="flex items-center gap-1.5 mr-2">
              ...
              >
                ...
                <SelectPopover>
                  <SelectContent aria-label="items" className="px-2 py-2">
                    {[...storageManager.getAllFormatSettings().values()].map(
                      (fs: FormatSettings) => {
                        return (
                          <SelectItem
                            textValue={fs.name}
            ...
            {currentFormatIsBuiltIn ? (
              <Button
                size="md"
                aria-label="Reset Format to Defaults"
                onPress={handleReset}
              >
                Reset
              </Button>
...
const OpenAIConnectionSettings = () => {
  const connectionSettings =
    storageManager.getOpenAIConnectionSettingsById("OPENAI");
    ...
    <>
      <div>
        <TextField className="flex max-w-[700px] items-center gap-1.5 mr-2">
          <Label className="w-[320px] text-md">
            Server URL
            <br />
        ...
        Supports any server compatible with the OpenAI 'completions' endpoint
        (e.g. <strong>Oogabooga</strong>).
      </div>
...
const DummyValueSettings = () => {
  const connectionSettings =
    storageManager.getDummyConnectionSettingsById("DUMMY");
    ...
    <>
      <div>
        <TextField className="flex items-center gap-1.5 mr-2">
          ...
          <TextAreaAutosizeJolly
            className="min-h-[100px]"
            value={placeholder}
            onChange={handlePlaceholderUpdate}
          />
        </TextField>
        For testing. No attempt to connect to an AI will be made. The dummy
        response will be returned to all messages.
      </div>
    </>
  );
};
...
const FormatSettingsEditor = ({
  formatSettings,
  setFormatSettings,
  formatSettingsId,
}: FormatSettingsEditorProps) => {
  ...
  const handleTextAreaUpdate = (
    ...
    if (field === "name") {
      ...
      nextFormatSettings = {
        ...formatSettings,
        ...{ systemPrefix: event.target.value },
      };
    } else if (field === "systemSuffix") {
      nextFormatSettings = {
        ...formatSettings,
        ...{ systemSuffix: event.target.value },
      };
    } else if (field === "userPrefix") {
      nextFormatSettings = {
        ...formatSettings,
        ...{ userPrefix: event.target.value },
      };
    } else if (field === "userSuffix") {
      nextFormatSettings = {
        ...formatSettings,
        ...{ userSuffix: event.target.value },
      };
    } else if (field === "lastUserPrefix") {
      nextFormatSettings = {
        ...formatSettings,
        ...{ lastUserPrefix: event.target.value },
      };
    } else if (field === "lastUserSuffix") {
      nextFormatSettings = {
        ...formatSettings,
        ...{ lastUserSuffix: event.target.value },
      };
    } else if (field === "assistantPrefix") {
      nextFormatSettings = {
        ...formatSettings,
        ...{ assistantPrefix: event.target.value },
      };
    } else if (field === "assistantSuffix") {
      nextFormatSettings = {
        ...formatSettings,
        ...{ assistantSuffix: event.target.value },
      };
    } else if (field === "lastAssistantPrefix") {
      nextFormatSettings = {
        ...formatSettings,
        ...{ lastAssistantPrefix: event.target.value },
      };
    } else if (field === "instructionFormat") {
      nextFormatSettings = {
        ...formatSettings,
        ...{ instructionFormat: event.target.value },
      };
    } else {
      throw Error(
        `FormatSettingsEditor.handleTextAreaUpdate called for ${field}`
      );
    }
  ...
  const textareaField = (field: keyof FormatSettings) => {
    return (
      <TextField className="flex items-center gap-1.5 mr-2 my-1">
        ...
        <TextAreaAutosizeJolly
          value={formatSettings[field]}
          onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
            handleTextAreaUpdate(e, field)
          }
        />
      </TextField>
    );
  ...
  const handleChatRoleChange = (key: Key, field: keyof FormatSettings) => {
    ...
    if (chatRole === undefined) {
      throw Error(
        `FormatSettingsEditor.handleChatRoleChange got undefined chatRole`
      );
    }
    var nextFormatSettings: FormatSettings = formatSettings;
    if (field === "authorsNoteRole") {
      ...
      throw Error(
        `FormatSettingsEditor.handleChatRoleChange called for ${field}`
      );
    }
    setFormatSettings(nextFormatSettings);
    storageManager.setFormatSettings(formatSettingsId, nextFormatSettings);
      ...
      <TextField className="flex items-center gap-1.5 mr-2 my-1">
        ...
        <Select
          ...
          onSelectionChange={(key: Key) => {
            handleChatRoleChange(key, field);
          }}
