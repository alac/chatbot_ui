...
const LorebookPanel = () => {
  const [lorebookUsageUpdatedTimestamp, setLorebookUsageUpdatedTimestamp] =
    useState(new Date());
  useEffect(() => {
      ...
      <div className="flex items-center">
        ...
        <div className="ml-auto">
          <span className="corner-button">
            <ViewUsedEntriesButton
              lorebookUsageUpdatedTimestamp={lorebookUsageUpdatedTimestamp}
              key={`lbEntry_${lorebookUsageUpdatedTimestamp}`}
            />
          </span>
          <span className="corner-button">
            <ViewLorebooksButton />
...
const ViewLorebooksButton = () => {
  const [lorebookUpdateTimestamp, setLorebookUpdateTimestamp] = useState(
    new Date()
  );
  useEffect(() => {
  ...
  const handleNewLorebookInputChange = (
  ...
  const handleReorderLorebook = (lorebookId: string, increment: number) => {
    ...
    const lorebookIdsWithoutX = storageManager.storageState.lorebookIds.filter(
      (s: string) => s !== lorebookId
    );
    const reorderedLorebookIds = [
      ...lorebookIdsWithoutX.slice(0, index + increment),
      lorebookId,
      ...lorebookIdsWithoutX.slice(index + increment),
    ];
  ...
  const handleMaxEntriesInputChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
  ...
  const handleMaxLorebookTokensInputChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const numberValue = parseInt(event.target.value, 10);
    if (!isNaN(numberValue) && numberValue >= -1) {
  ...
  const lorebooks = storageManager.storageState.lorebookIds.map((s: string) =>
    storageManager.lorebooks.get(s)
  );
  const enabledLorebookIds = storageManager.currentConversation.lorebookIds;
  const handleLorebookSelectionChange = (
    lorebookId: string,
    isSelected: boolean
  ) => {
    if (
      isSelected &&
      !storageManager.currentConversation.lorebookIds.includes(lorebookId)
    ) {
      storageManager.currentConversation.lorebookIds = [
        ...storageManager.currentConversation.lorebookIds,
        lorebookId,
      ];
    ...
    if (
      ...
      storageManager.currentConversation.lorebookIds =
        storageManager.currentConversation.lorebookIds.filter(
          (s: string) => s !== lorebookId
        );
      storageManager.persistConversation();
      storageManager.lorebookUpdatedCallback?.();
  ...
  return (
    <DialogTrigger>
      ...
      <DialogOverlay>
        <DialogContent
          className="max-w-[80%] max-h-[90%] overflow-y-scroll"
          isDismissable={true}
        >
          <DialogHeader>
          ...
          <span className="text-md">
            Use lorebooks to conditionally include text in requests.
          </span>
          <span>
            Example: Suppose you had a lorebook entry with a trigger on
            "Hogwarts" and definition of "Hogwarts School of Witchcraft and
            Wizardry is a prestigious magical academy...". Before "Hogwarts" is
            mentioned, no extra text would be included. On the other hand,
            mentioning "Hogwarts" would add the definition to the conversation.
          </span>
          <div
            className="flex items-center"
            key={`lbentries_${lorebookUpdateTimestamp}`}
          >
            <TextField className="flex max-w-sm items-center gap-1.5 mr-2">
              <Label className="py-2">
                Max Active Lorebook Entries (default: 10):{" "}
              </Label>
              <Input
                placeholder={`${maxLorebookEntries}`}
                onBlur={handleMaxEntriesInputChange}
              />
            </TextField>

            <TextField className="flex max-w-sm items-center gap-1.5">
              <Label className="py-2">
                Max Tokens for Lorebook Entries (default: 1000):{" "}
              </Label>
              <Input
                placeholder={`${maxLorebookTokens}`}
                onBlur={handleMaxLorebookTokensInputChange}
              />
            </TextField>
          </div>
          <div>
            0 in either field disables lorebook insertion{" "}
            <strong>entirely</strong>. -1 disables just that constraint.{" "}
          </div>
          ...
          <TextField className="flex max-w-lg items-center gap-1.5">
            ...
            <Input
              placeholder="Gandalf and his homies"
              onInput={handleNewLorebookInputChange}
            />
            <Button
              size="md"
              aria-label="New Lorebook"
              onPress={handleCreateLorebook}
            >
              Create Lorebook{" "}
            </Button>
          </TextField>
            ...
            <div className="grid gap-2">
              {lorebooks.map((lb: Lorebook | undefined, index: number) => {
                ...
                return (
                  ...
                  >
                    <Checkbox
                      defaultSelected={enabledLorebookIds.includes(
                        lb.lorebookId
                      )}
                      onChange={(isSelected: boolean) =>
                        handleLorebookSelectionChange(lb.lorebookId, isSelected)
                      }
                    >
                      {lb.lorebookName}
                    </Checkbox>
                    <div className="ml-auto">
                      <span className="corner-button mx-2">
                        <EditLorebookButton lorebookId={lb.lorebookId} />
                      </span>
                      {/* Export <Button size="icon"><Export /></Button> */}
                      <span className={"corner-button mx-2" + hideUp}>
                        <Button
                          size="icon"
                          aria-label="Move lorebook up in list"
                          onPress={() =>
                            handleReorderLorebook(lb.lorebookId, -1)
                          }
                        >
                          <ChevronUp />
                        </Button>
                      </span>
                      <span className={"corner-button mx-2" + hideDown}>
                        <Button
                          size="icon"
                          aria-label="Move lorebook down in list"
                          onPress={() =>
                            handleReorderLorebook(lb.lorebookId, 1)
                          }
                        >
                      ...
                      <span className="corner-button mx-2">
                        <Button
                          ...
                          onPress={() => handleDeleteLorebook(lb.lorebookId)}
                        >
                          <Delete />
                        </Button>
                      </span>
    ...
    <DialogTrigger>
      ...
      <DialogOverlay>
        ...
        >
          <DialogHeader>
            <DialogTitle>
              Editing Lorebook: '{lorebook.lorebookName}'
            </DialogTitle>
          </DialogHeader>
          ...
          <Button
            size="md"
            aria-label="New Lorebook"
            onPress={handleCreateLorebookEntry}
          >
            +
          </Button>
          <div key={`${lorebookUpdateCount}`}>
            <div className="grid gap-2">
              {lorebook.lorebookEntry.map(
                (le: LorebookEntry | undefined, index: number) => {
                  if (le === undefined) {
                    return <></>;
                  }
                  return (
...
const LorebookEntryEditor = ({
  ...
  const handleEntryTriggerChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
  ...
  return (
    <div
      className="flex flex-col hover:bg-gray-200 transition duration-300 ease-in-out px-2 pb-2 gap-1"
      key={lorebookEntry.entryId}
    >
      <div className="flex items-center gap-1.5">
        ...
        <Checkbox
          defaultSelected={lorebookEntry.isEnabled}
          onChange={(event) => {
            handleEntryEnabledChange(event);
          }}
        >
          <Label className="py-2">Enable</Label>
        </Checkbox>
      </div>
      <TextField className="flex items-center gap-1.5">
        ...
        <Input
          ...
          onBlur={(event) => {
            handleEntryTriggerChange(event);
          }}
        />
      </TextField>
...
const ViewUsedEntriesButton = ({
  ...
  lorebookUsageUpdatedTimestamp: Date;
}) => {
    ...
    content = (
      <>
        <span
          ...
          key={`lbEntrySummary_${lorebookUsageUpdatedTimestamp}`}
        >
          Total Tokens: {Math.round(generateStatsTracker.lorebookUsageTokens)} /{" "}
          {maxTokens}.
        </span>
        <div>
          <span className="font-medium">
            Active Entries ({generateStatsTracker.lorebookUsageEntries.length} /{" "}
            {maxEntries}):{" "}
          </span>
        </div>
        {generateStatsTracker.lorebookUsageEntries.map(
          (le: LorebookEntry, index: number) => {
            const tokenCount = generateStatsTracker.getTokenCount(le);
            if (tokenCount !== -1) {
              return (
                <div>
                  <span className="font-medium">- {le.entryName}</span> (
            ...
            return (
              <div>
                <span className="font-medium">{le.entryName}</span> (N/A).
              </div>
            );
          }
        )}
  ...
  return (
    <PopoverTrigger>
      <Button size="icon" aria-label="View Lorebook Usage">
        <Magnify />
      </Button>
      <Popover placement="start">{content}</Popover>
    </PopoverTrigger>
  );
};
