  ...
  const promptButton = (
    <DialogTrigger>
      <span className="corner-button">
        <Button variant="outline" size="icon" aria-label="Show Prompt">
          <Compare />
        </Button>
      </span>
      <DialogOverlay>
    ...
    <div style={{ paddingBottom: "1rem", display: "flex" }} key={data.key}>
      <div
        style={{
          ...
          background: ownMessage
            ? `${data.isDisabled ? "#d4d3d3" : "#27272a"}`
            : `${data.isDisabled ? "#e2e8f0" : "#e2e8f0"}`,
          color: ownMessage
            ? `${data.isDisabled ? "#fafafa" : "white"}`
            : `${data.isDisabled ? "#94a3b8" : "#1e293b"}`,
          borderRadius: "1rem",
          padding: "1rem",
          whiteSpace: "pre",
          textWrap: "wrap",
      ...
      >
        ...
        <EditableText
          ...
          onTextChange={updateMessageText}
          isEditing={isEditing}
  ...
  return (
    ...
    >
      {isEditing ? (
        ...
        <span ref={spanRef} className="message-body">
