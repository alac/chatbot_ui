...
export function isConversation(obj: any): obj is Conversation {
  return (
    ...
    "editEvents" in obj &&
    Array.isArray(obj.editEvents) &&
    obj.editEvents.every((editEvent: any) => isEditEvent(editEvent)) &&
    "editEventsRedoQueue" in obj &&
    Array.isArray(obj.editEventsRedoQueue) &&
...
const isEditEventAdd = (obj: any): obj is EditEventAdd => {
  return (
    ...
    obj !== null &&
    "editId" in obj &&
    typeof obj.editId === "number" &&
    "type" in obj &&
    [EventType.Add].includes(obj.type) &&
    "addMessage" in obj &&
    typeof obj.addMessage === "object" &&
    isMessage(obj.addMessage)
  );
};
...
const isEditEventUpdate = (obj: any): obj is EditEventUpdate => {
  return (
    ...
    typeof obj.editId === "number" &&
    "type" in obj &&
    [EventType.Update].includes(obj.type) &&
    "updateMessage" in obj &&
    typeof obj.updateMessage === "object" &&
    "key" in obj.updateMessage &&
    typeof obj.updateMessage.key === "string"
  );
};
...
const isEditEventDelete = (obj: any): obj is EditEventDelete => {
  return (
    ...
    typeof obj.editId === "number" &&
    "type" in obj &&
    [EventType.Delete].includes(obj.type) &&
    "deleteKey" in obj &&
    typeof obj.deleteKey === "string"
  );
};
...
const isEditEvent = (obj: any): obj is EditEvent => {
  return (
    ...
    typeof obj.editId === "number" &&
    "type" in obj &&
    [EventType.Add, EventType.Delete, EventType.Update].includes(obj.type) &&
...
const isStorageState = (obj: unknown): obj is StorageState => {
  if (
    (typeof obj === "object" &&
      ...
      (obj.currentConversationId === null ||
        typeof obj.currentConversationId === "string") &&
      ...
      obj.formatSettingsById instanceof Map) === false
  ) {
...
function getDummyConnectionSettings(): DummyConnectionSettings {
  return {
    type: "DUMMY",
    response:
      "Click the edit button on the connections panel to set-up a connection to the AI",
  };
}
...
const isFormatSettings = (obj: any): obj is FormatSettings => {
  return (
    ...
    "instructionFormat" in obj &&
    typeof obj.instructionFormat === "string" &&
    "systemMessage" in obj &&
    typeof obj.systemMessage === "string" &&
    "systemPrefix" in obj &&
    typeof obj.systemPrefix === "string" &&
    "systemSuffix" in obj &&
    typeof obj.systemSuffix === "string" &&
    "userPrefix" in obj &&
    typeof obj.userPrefix === "string" &&
    "userSuffix" in obj &&
    typeof obj.userSuffix === "string" &&
    "lastUserPrefix" in obj &&
    typeof obj.lastUserPrefix === "string" &&
    "lastUserSuffix" in obj &&
    typeof obj.lastUserSuffix === "string" &&
    "assistantPrefix" in obj &&
    typeof obj.assistantPrefix === "string" &&
    "assistantSuffix" in obj &&
    typeof obj.assistantSuffix === "string" &&
    "lastAssistantPrefix" in obj &&
    typeof obj.lastAssistantPrefix === "string" &&
    "authorsNoteRole" in obj &&
    [ChatRole.Bot, ChatRole.System, ChatRole.User].includes(
      obj.authorsNoteRole
    ) &&
    "lorebookRole" in obj &&
    [ChatRole.Bot, ChatRole.System, ChatRole.User].includes(obj.lorebookRole)
  );
};
...
function getDefaultFormatSettingsMap(): Map<string, FormatSettings> {
  ...
  const defaultFormat = {
    ...
    systemMessage:
      "You are {{char}}, a friendly and creative assistant, prone to making wild assertions and unlikely connections. As {{char}}, continue the exchange with {{user}}.",
...
class StorageManager {
  ...
  constructor() {
    const defaultConnections = new Map<
      string,
      DummyConnectionSettings | OpenAIConnectionSettings
    >();
  ...
  startup(): void {
    localforage
      ...
      .then(() => {
        localforage
          .getItem(STORAGE_STATE_KEY, (err, readValue) => {
            ...
            if (err !== null) {
              console.log(
                "Error in StorageManager reading STORAGE_STATE_KEY: ",
                err
              );
            }
          })
          .then(() => {
            this.storageState.conversationIds.map((conversationId: string) => {
              localforage.getItem(conversationId, (err, readValue) => {
                if (isConversation(readValue)) {
                  ...
                  if (
                    this.storageState.currentConversationId !== null &&
                  ...
                  console.log(
                    "isConversation typeguard failed; value: ",
                    readValue
                  );
                }
                if (err !== null) {
                  console.log(
                    "Error in StorageManager reading conversations: ",
                    err
                  );
                }
              });
            ...
            this.storageState.lorebookIds.map((lorebookId: string) => {
              localforage.getItem(lorebookId, (err, readValue) => {
                ...
                if (err !== null) {
                  console.log(
                    "Error in StorageManager reading lorebooks: ",
                    err
                  );
                }
              });
  ...
  setActiveConversation(conversationId: string): void {
    if (this.conversations.has(conversationId)) {
      ...
      if (newConversation !== undefined) {
        ...
        this.storageState.currentConversationId = conversationId;
        this.messagesCurrent = this.applyEditEvents(
          [],
          this.currentConversation.editEvents
        );
  ...
  deleteConversation(conversationId: string): void {
    ...
    this.storageState.conversationIds =
      this.storageState.conversationIds.filter(
  ...
  updateMessage(message: Message, rerender: boolean = true): void {
    ...
    const allMessageIds = this.messagesCurrent.map(
      (someMessage: Message) => someMessage.key
    );
  ...
  deleteMessage(messageId: string): void {
    const newMessageList = this.messagesCurrent.filter(
      (someMessage: Message) => someMessage.key !== messageId
    );
  ...
  createDeleteEditEvent(messageId: string): void {
    const prevMatches = this.messagesPrevious.filter(
      (m: Message) => m.key === messageId
    );
    if (prevMatches.length === 0) {
      console.error(
        "createUpdateEditEvent called for a message that does not have any match"
      );
      return;
    } else if (prevMatches.length > 1) {
      console.error(
        "createUpdateEditEvent called for a message that has multiple matches"
      );
      return;
    }
    ...
    for (var i = 0; i < editEvents.length; i++) {
      ...
      if (isEditEventDelete(editEvent)) {
        ...
        messages = messages.filter(
          (m: Message) => m.key !== deleteEvent.deleteKey
        );
      }
  ...
  deleteLorebook(lorebookId: LorebookId): void {
    ...
    this.storageState.lorebookIds = this.storageState.lorebookIds.filter(
  ...
  updateLorebookOrder(lorebookIds: string[]): void {
    const validNewIds = lorebookIds.filter((s: string) =>
      this.storageState.lorebookIds.includes(s)
    );
    const missingOldIds = this.storageState.lorebookIds.filter(
      (s: string) => !lorebookIds.includes(s)
    );
  ...
  getCurrentConnectionSettings(): AnyConnectionSettings {
    const settings = this.storageState.connectionSettingsById.get(
      this.storageState.currentConnectionSettingsId
    );
    if (settings) {
      return settings;
    }
    throw Error(
      "Invalid currentConnectionSettingsId: " +
        this.storageState.currentConnectionSettingsId
    );
  }
  ...
  getCurrentFormatSettings(): FormatSettings {
    const settings = this.storageState.formatSettingsById.get(
      this.storageState.currentFormatSettingsId
    );
    if (settings) {
      return settings;
    }
    throw Error(
      "Invalid currentFormatSettingsId: " +
        this.storageState.currentFormatSettingsId
    );
  }
  ...
  setCurrentSamplingSettingsId(id: string): void {
    if (!this.storageState.samplingSettingsById.has(id)) {
      console.error(
        `setCurrentSamplingSettingsId called for missing id: ${id}`
      );
      return;
    }
